...
NULL means nothing, but NULL is not “Nothing.” NULL means that “a data value does not exist in the database” ([Source](https://en.wikipedia.org/wiki/Null_(SQL))). This means that the value simply isn't there; it does **not** mean that the value is 0,  N/A, or False. N/A, for all its ambiguity, is a value. Resisting the temptation to create a default value for a field may be counter-intuitive, but using NULL correctly can benefit both the end user and you.
