...
When you use ```select *``` and your intuition, this table makes it clear that I adopted two cats. However, this is a small table; in bigger tables we are unable to view all records at once. As a result, users often use aggregates such as:
...
Notice that, within this one query, the user has to know the distinct default values for two fields. This is asking a lot of the user and setting them up for potential incorrect results.
...
#### Default Behavior
Assume our table was designed like this:
...
The following are examples of NULL functions that would work if our table looked like this:

```
SELECT COUNT(*) FROM cat_adoption_fact;
```
By default, Snowflake will not count any NULL values by default ([source](https://docs.snowflake.net/manuals/sql-reference/functions/count.html)), so this will return 2. I only have 2 cats.
...
``` 
Using NULL actually gives you more control over [ORDER BY](https://docs.snowflake.net/manuals/sql-reference/constructs/order-by.html). Window functions even have a feature to IGNORE NULLs [example](https://docs.snowflake.net/manuals/sql-reference/functions/lag.html). 
