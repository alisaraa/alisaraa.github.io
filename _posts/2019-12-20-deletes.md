...
When compared to the source, this is wrong becuase it still has Sushi's hospitalization. For whatever reason, Sushi’s hospitalization was deleted. In order to have known this, we would have had to set the extraction date back to 2019-12-18, but, since we have no way of knowing which records will be deleted when, we’d actually have to set the extraction date back to the beginning of time to accounr for hard deletes. This would just be a full load.
